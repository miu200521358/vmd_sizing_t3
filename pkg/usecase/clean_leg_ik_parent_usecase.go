...
func CleanLegIkParent(sizingSet *domain.SizingSet, setSize int) (bool, error) {
	...
	if err := miter.IterParallelByList(frames, blockSize, log_block_size, func(data, index int) {
		...
		mlog.I(mi18n.T("足IK親最適化01", map[string]interface{}{"No": sizingSet.Index + 1, "IterIndex": fmt.Sprintf("%04d", iterIndex), "AllCount": fmt.Sprintf("%02d", allCount), "Progress": fmt.Sprintf("%.2f", float64(iterIndex)/float64(allCount)*100)}))
	}); err != nil {
		sizingMotion.Processing = false
		return false, err
	}
	...
	for i, endFrame := range frames {
		...
		if endFrame > logEndFrame {
			mlog.I(mi18n.T("足IK親最適化02", map[string]interface{}{"No": sizingSet.Index + 1, "IterIndex": fmt.Sprintf("%04d", endFrame), "AllCount": fmt.Sprintf("%04d", allCount), "Progress": fmt.Sprintf("%.2f", float64(endFrame)/float64(allCount)*100)}))
