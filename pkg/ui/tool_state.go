...
type ToolState struct {
	...
	SizingTabMotionSaveButton     *walk.PushButton    // サイジングタブモーション保存ボタン
	SizingTabModelSaveButton      *walk.PushButton    // サイジングタブモデル保存ボタン
	currentPageChangedPublisher   walk.EventPublisher
	JsonSaveTab                   *widget.MTabPage // サイジングタブページ
}

type loadVmdResult struct {
...
func (toolState *ToolState) setCurrentAction(index int) error {
	...
	toolState.SizingReductionCheck.UpdateChecked(sizingSet.IsSizingReduction)
...
func (toolState *ToolState) SetSizingCheckEnabled(enabled bool) {
	...
	toolState.SizingReductionCheck.SetEnabled(enabled)
...
func (toolState *ToolState) ResetSizingCheck(value bool) {
	...
	toolState.SizingReductionCheck.UpdateChecked(value)
